		...
		if( event.Type == UIEVENT_BUTTONUP )
		{
			if( (m_uButtonState & UISTATE_CAPTURED) != 0 ) {
				...
				if (::PtInRect(&m_rcItem, event.ptMouse)) {
					...
					LPCTSTR pszClickReportID = GetClickReportID();
					if (pszClickReportID && *pszClickReportID && m_pManager)
						::SendMessage(m_pManager->GetPaintWindow(), UIMSG_CLICK_REPORT, 0, (LPARAM)pszClickReportID);
				}
			}
			return;
		}
