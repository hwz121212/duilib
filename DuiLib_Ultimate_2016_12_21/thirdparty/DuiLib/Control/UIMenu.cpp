...
namespace DuiLib {
	...
	void CMenuElementUI::DrawItemIcon(HDC hDC, const RECT& rcItem)
	{
		if (!m_strIcon.IsEmpty() && !(m_bCheckItem && !GetChecked()))
		{
			...
			if ((m_uButtonState & UISTATE_HOT) && !m_strHotIcon.IsEmpty())
				DrawImage(hDC, m_strHotIcon, pStrImage);
			else
	...
	void CMenuElementUI::SetHotIcon(LPCTSTR strHotIcon)
	{
		if (strHotIcon != _T(""))
			m_strHotIcon = strHotIcon;
	}
	...
	void CMenuElementUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if (_tcsicmp(pstrName, _T("hoticon")) == 0) {
			SetHotIcon(pstrValue);
		}
