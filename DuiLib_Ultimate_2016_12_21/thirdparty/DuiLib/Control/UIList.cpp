...
namespace DuiLib {
	...
	CListUI::CListUI() : m_pCallback(NULL), m_bScrollSelect(false), m_iCurSel(-1), m_iExpandedItem(-1), m_bMultiSel(false)
	{
		...
		m_ListInfo.bAlternateItemBkLast = true;
	...
	void CListUI::SetAlternateItemBkLast(bool bAlternateItemBkLast)
	{
		m_ListInfo.bAlternateItemBkLast = bAlternateItemBkLast;
		Invalidate();
	}
	...
	void CListUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if (_tcsicmp(pstrName, _T("itemaltbklast")) == 0) SetAlternateItemBkLast(_tcsicmp(pstrValue, _T("true")) == 0);
	...
	void CListContainerElementUI::DrawItemBk(HDC hDC, const RECT& rcItem)
	{
		...
		//libing
		if (!pInfo->bAlternateItemBkLast) {
			if (!pInfo->bAlternateBk || m_iIndex % 2 == 0) iBackColor = pInfo->dwBkColor;  //隔行背景以itembk背景为先，大背景为后
		} else {
			if (!pInfo->bAlternateBk || m_iIndex % 2 != 0) iBackColor = pInfo->dwBkColor;  //隔行背景以大背景为先，itembk为后
		}
