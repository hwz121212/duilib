...
namespace DuiLib
{
	...
	LRESULT CEditWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
        ...
        //{{
			...
			m_pOwner->GetManager()->SendNotify(m_pOwner, DUI_MSGTYPE_VK_DOWN);
        } 
        else if( uMsg == WM_KEYDOWN && TCHAR(wParam) == VK_UP) {
			m_pOwner->GetManager()->SendNotify(m_pOwner, DUI_MSGTYPE_VK_UP);
        } 
        else if( uMsg == WM_KEYDOWN && TCHAR(wParam) == VK_ESCAPE) {
			m_pOwner->GetManager()->SendNotify(m_pOwner, DUI_MSGTYPE_VK_ESCAPE);
        ...
        //}}
			...
			if (TCHAR(wParam) == VK_RETURN) {  //为了屏蔽edit控件回车发出的声音
				return 0;
			}
			bHandled = FALSE;
		}
	...
	LRESULT CEditWnd::OnKillFocus(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
        ...
        m_pOwner->GetManager()->SendNotify(m_pOwner, DUI_MSGTYPE_KILLFOCUS, wParam);
		return lRes;
	}
