...
namespace DuiLib {
	...
	void CControlUI::SetVisible(bool bVisible)
	{
        ...
        // 显示状态改变时通知 by lizhou.wang
        if (m_pManager && v != IsVisible()) {
            m_pManager->SendNotify(this, DUI_MSGTYPE_VISIBLE_CHANGED, IsVisible() ? TRUE : FALSE);
        }
	...
	void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcsicmp(pstrName, _T("clickreportid")) == 0) SetClickReportID(pstrValue);
		else {
