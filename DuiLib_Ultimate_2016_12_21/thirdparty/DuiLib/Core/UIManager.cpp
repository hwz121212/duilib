...
namespace DuiLib {
		...
		m_bStopPaint(false)
	{
        ...
        unfocous_wnd_ = false; // by lizhou.wang
        is_focus_lock_ = false;
	}
    ...
    void CPaintManagerUI::DeletePtr(void*& ptr)//by lizhou.wang
	{
	...
	bool CPaintManagerUI::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT& lRes)
	{
		...
		case WM_CLOSE:
			{
				...
				if( ::GetActiveWindow() == m_hWndPaint ) {
                    ...
                    // by lizhou.wang
					if( hwndParent != NULL ) {
                        if (!unfocous_wnd_) ::SetFocus(hwndParent);
                    }
				}
		...
		case WM_ERASEBKGND:
			...
			//return true; for test by rejoewang
			return false;
		case WM_PAINT:
			{
				if (m_bStopPaint) return true;
		...
		case WM_MOUSEMOVE:
			{
				...
				if(bNeedDrag && m_bDragMode && wParam == MK_LBUTTON)
				{
					...
					DWORD dwDragControlEffect = DROPEFFECT_NONE;
					ASSERT(m_pEventDrag);
					if (m_pEventDrag) {
						if (m_pEventDrag->IsDropCopyEnabled()) {
							dwDragControlEffect |= DROPEFFECT_COPY;
						} 
						if (m_pEventDrag->IsDropMoveEnabled()) {
							dwDragControlEffect |= DROPEFFECT_MOVE;
						}
						pdsrc->is_use_default_cursor_ = m_pEventDrag->IsDefaultDragCursorEnabled();
					} 
					//HRESULT hr = ::DoDragDrop(pdobj, pdsrc, DROPEFFECT_COPY | DROPEFFECT_MOVE, &dwEffect);
					HRESULT hr = ::DoDragDrop(pdobj, pdsrc, dwDragControlEffect, &dwEffect);
					pdsrc->Release();
					pdobj->Release();
					m_bDragMode = false;
					...
					if (m_pEventDrag)
					{
						SendNotify(m_pEventDrag, DUI_MSGTYPE_DRAGRELEASE, ptNow.x, ptNow.y);
						m_pEventDrag = NULL;
					}
					break;
				}
		...
		case WM_LBUTTONDOWN:
			{
                ...
                // by lizhou.wang
                if (!unfocous_wnd_) ::SetFocus(m_hWndPaint);
		...
		case WM_KEYDOWN:
			{
				if( m_pFocus == NULL ) break;
				if (unfocous_wnd_ == true) return true; // add by caesarwu
				TEventUI event = { 0 };
	...
	void CPaintManagerUI::ReapObjects(CControlUI* pControl)
	{
		...
		if( pControl == m_pEventDrag ) m_pEventDrag = NULL;
		KillTimer(pControl);
	...
	void CPaintManagerUI::SetStopPaint(bool bFlag)
	{
		m_bStopPaint = bFlag;
	}
