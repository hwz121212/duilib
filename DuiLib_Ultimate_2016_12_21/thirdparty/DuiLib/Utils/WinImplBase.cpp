...
namespace DuiLib
{
	...
	LRESULT WindowImplBase::OnGetMinMaxInfo(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
        ...
        if (maxsize.cx && maxsize.cy) { 
            lpMMI->ptMaxTrackSize.x = maxsize.cx;
            lpMMI->ptMaxTrackSize.y = maxsize.cy;
        }
	...
	LRESULT WindowImplBase::OnEraseBkgnd(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
	{
		bHandled = TRUE;// 默认不绘制背景???
		return 1;
	}
